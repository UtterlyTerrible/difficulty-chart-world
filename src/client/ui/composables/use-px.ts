...
const BASE_RESOLUTON = new Vector2(1920, 1080);
const MIN_SCALE = 0.5;
...
export function usePx() {
	...
	const updateScale = () => {
		const width = math.log(camera.ViewportSize.X / BASE_RESOLUTON.X, 2);
		const height = math.log(camera.ViewportSize.Y / BASE_RESOLUTON.Y, 2);
